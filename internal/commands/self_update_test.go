...
func TestSelfUpdateCheckReportsNewerAvailable(t *testing.T) {
	...
	// --check must exit 0 when an update is available (apt-style: "work to do").
	if pendingExitCode != 0 {
		t.Errorf("expected pendingExitCode=0 when update available, got %d", pendingExitCode)
	}
}

// --check exits selfUpdateCheckExitCurrent (100) when the running binary is
// already the latest release — lets scripts distinguish "nothing to do"
// from real errors.
func TestSelfUpdateCheckExits100WhenCurrent(t *testing.T) {
	mock := NewMockClient()
	SetTestModeWithSDK(mock)
	defer resetTest()

	const tag = "v3.0.3-tt.8.gfc188b6"
	...
	if pendingExitCode != selfUpdateCheckExitCurrent {
		t.Errorf("expected pendingExitCode=%d, got %d",
			selfUpdateCheckExitCurrent, pendingExitCode)
	}
}

// --check on a released binary (reports version without leading "v") that
// matches the GitHub tag (with leading "v") must still exit 100.
func TestSelfUpdateCheckExits100AcrossVPrefix(t *testing.T) {
	mock := NewMockClient()
	SetTestModeWithSDK(mock)
	defer resetTest()
	...
	const tag = "v3.0.3-tt.8.gfc188b6"
	newSelfUpdateFixture(t, tag)
	withVersion(t, "3.0.3-tt.8.gfc188b6") // no "v"
	resetSelfUpdateFlags(t)
	selfUpdateCheck = true
...
func TestSelfUpdateNoOpWhenAlreadyCurrent(t *testing.T) {
	...
	// bare `self-update` (no --check) must always exit 0, even on no-op.
	if pendingExitCode != 0 {
		t.Errorf("expected pendingExitCode=0 for bare self-update no-op, got %d", pendingExitCode)
	}
}

// Released binaries report cliVersion without a leading "v" (goreleaser's
// {{.Version}} strips it), while the GitHub tag keeps it. Self-update must
// treat these as the same version so re-running is a no-op.
func TestSelfUpdateTreatsVPrefixAsSameVersion(t *testing.T) {
	mock := NewMockClient()
	SetTestModeWithSDK(mock)
	defer resetTest()
	...
	const tag = "v3.0.3-tt.8.gfc188b6"
	newSelfUpdateFixture(t, tag)
	withVersion(t, "3.0.3-tt.8.gfc188b6") // note: no leading "v"
	resetSelfUpdateFlags(t)
	...
	data, _ := lastResult.Response.Data.(map[string]any)
	if data["updated"] != false {
		t.Errorf("expected updated=false (no-op on v/non-v match), got %#v", data)
	}
}

func TestNormalizeTag(t *testing.T) {
	cases := []struct{ in, want string }{
		{"v3.0.3-tt.8.gfc188b6", "3.0.3-tt.8.gfc188b6"},
		{"3.0.3-tt.8.gfc188b6", "3.0.3-tt.8.gfc188b6"},
		{"", ""},
		{"v", ""},
	}
	for _, c := range cases {
...
func TestSelfUpdateSwapsBinary(t *testing.T) {
	...
	// Successful update must also exit 0 (no 100-ish signal when an
	// actual update happened).
	if pendingExitCode != 0 {
		t.Errorf("expected pendingExitCode=0 after successful update, got %d", pendingExitCode)
	}
}
...
func TestInsideFizzyWorktree(t *testing.T) {
	// Positive: fizzy-cli worktree — go.mod at the repo root declares the
	// fizzy-cli module, binary lives in bin/.
	fizzyRoot := t.TempDir()
	if err := os.WriteFile(filepath.Join(fizzyRoot, "go.mod"),
		[]byte("module github.com/basecamp/fizzy-cli\n\ngo 1.26\n"), 0o644); err != nil {
		t.Fatalf("write fizzy go.mod: %v", err)
	}
	fizzyBin := filepath.Join(fizzyRoot, "bin")
	if err := os.Mkdir(fizzyBin, 0o755); err != nil {
		t.Fatalf("mkdir bin: %v", err)
	}
	fizzyExe := filepath.Join(fizzyBin, "fizzy")
	if err := os.WriteFile(fizzyExe, []byte("x"), 0o755); err != nil {
		t.Fatalf("write fizzy exe: %v", err)
	}
	if !insideFizzyWorktree(fizzyExe) {
		t.Errorf("insideFizzyWorktree(%q) = false, want true", fizzyExe)
	}

	// Negative: a different module's worktree (e.g. dotfiles with its own
	// go.mod). Self-update must NOT refuse here.
	otherRoot := t.TempDir()
	if err := os.WriteFile(filepath.Join(otherRoot, "go.mod"),
		[]byte("module example.com/dotfiles\n"), 0o644); err != nil {
		t.Fatalf("write other go.mod: %v", err)
	}
	otherBin := filepath.Join(otherRoot, "bin")
	if err := os.Mkdir(otherBin, 0o755); err != nil {
		t.Fatalf("mkdir other bin: %v", err)
	}
	otherExe := filepath.Join(otherBin, "fizzy")
	if err := os.WriteFile(otherExe, []byte("x"), 0o755); err != nil {
		t.Fatalf("write other exe: %v", err)
	}
	if insideFizzyWorktree(otherExe) {
		t.Errorf("insideFizzyWorktree(%q) = true, want false (different module)", otherExe)
	}

	// Negative: a worktree with no go.mod at all (plain dotfiles repo).
	plainRoot := t.TempDir()
	if err := os.Mkdir(filepath.Join(plainRoot, ".git"), 0o755); err != nil {
		t.Fatalf("mkdir .git: %v", err)
	}
	plainBin := filepath.Join(plainRoot, "bin")
	if err := os.Mkdir(plainBin, 0o755); err != nil {
		t.Fatalf("mkdir plain bin: %v", err)
	}
	plainExe := filepath.Join(plainBin, "fizzy")
	if err := os.WriteFile(plainExe, []byte("x"), 0o755); err != nil {
		t.Fatalf("write plain exe: %v", err)
	}
	if insideFizzyWorktree(plainExe) {
		t.Errorf("insideFizzyWorktree(%q) = true, want false (no go.mod)", plainExe)
	}
}

func TestGoModIsModule(t *testing.T) {
	cases := []struct {
		name   string
		goMod  string
		module string
		...
		{"fizzy", "module github.com/basecamp/fizzy-cli\n\ngo 1.26\n", "github.com/basecamp/fizzy-cli", true},
		{"whitespace", "  module   github.com/basecamp/fizzy-cli  \n", "github.com/basecamp/fizzy-cli", true},
		{"different", "module example.com/other\n", "github.com/basecamp/fizzy-cli", false},
		{"no module line", "go 1.26\n", "github.com/basecamp/fizzy-cli", false},
		{"empty", "", "github.com/basecamp/fizzy-cli", false},
	}
	for _, c := range cases {
		if got := goModIsModule([]byte(c.goMod), c.module); got != c.want {
			t.Errorf("%s: goModIsModule() = %v, want %v", c.name, got, c.want)
		}
	}
}
