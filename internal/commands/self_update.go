...
const (
	...
	// Exit code for `self-update --check` when the running binary is
	// already at the latest release. Picked outside the internal/errors
	// rubric (0–8) to avoid collision with real errors, matching apt's
	// "non-zero signals work to do / nothing to do" convention.
	selfUpdateCheckExitCurrent = 100
)
...
func runSelfUpdate(cmd *cobra.Command, args []string) error {
	...
	current := cliVersion
	// goreleaser's {{.Version}} strips the leading "v" when setting
	// main.version, so the running binary reports "3.0.3-..." while the
	// GitHub tag is "v3.0.3-...". Compare and display on normalized forms
	// so users see consistent strings (also matches `fizzy version`).
	currentDisp := normalizeTag(current)
	latestDisp := normalizeTag(latest.Tag)
	upToDate := currentDisp == latestDisp
	...
	if selfUpdateCheck {
		...
		if isHumanOutput() {
			if upToDate {
				writeOutputString(fmt.Sprintf("fizzy is up to date at %s.\n", latestDisp))
			} else {
				writeOutputString(fmt.Sprintf("Update available: %s → %s.\n", currentDisp, latestDisp))
			}
			captureResponse()
		} else {
			printSuccess(data)
		}
		// Exit 0 when an update is available, selfUpdateCheckExitCurrent
		// (100) when already current. Lets scripts branch via `if !fizzy
		// self-update --check; then …; fi` while distinguishing real
		// errors (3/6/7) from the "nothing to do" signal.
		if upToDate {
			pendingExitCode = selfUpdateCheckExitCurrent
		}
		return nil
	}

	if upToDate && !selfUpdateForce {
		data := map[string]any{
			"current": current,
			"latest":  latest.Tag,
			"updated": false,
	...
	if isHumanOutput() {
		writeOutputString(fmt.Sprintf("Updated fizzy: %s → %s\n  at %s\n", currentDisp, latestDisp, exe))
		captureResponse()
	} else {
		printSuccess(data)
	}
...
// normalizeTag strips an optional leading "v" so a running binary's version
// (goreleaser strips the prefix) compares equal to the GitHub tag it came
// from (the tag keeps the prefix).
...
func resolveExecutable() (string, error) {
	...
	if insideFizzyWorktree(resolved) {
		return "", &output.Error{
			Code: output.CodeUsage,
			Message: fmt.Sprintf(
				"refusing to replace a fizzy-cli build artifact: %s", resolved),
			Hint: "self-update replaces the binary it runs from, and this one lives inside a fizzy-cli source tree. " +
				"If ~/bin/fizzy is a symlink into that worktree, replace it with the released binary first: " +
...
// insideFizzyWorktree reports whether path lives inside the fizzy-cli source
// tree — detected by walking up for a go.mod whose module is
// github.com/basecamp/fizzy-cli. The narrow check protects `make build`
// output from being clobbered while still letting self-update work when the
// binary lives inside some other git worktree (dotfiles, nix config, …).
func insideFizzyWorktree(path string) bool {
	const fizzyModule = "github.com/basecamp/fizzy-cli"
	dir := filepath.Dir(path)
	for {
		if data, err := os.ReadFile(filepath.Join(dir, "go.mod")); err == nil {
			return goModIsModule(data, fizzyModule)
		}
...
// goModIsModule reports whether a go.mod file declares the given module path.
func goModIsModule(goMod []byte, want string) bool {
