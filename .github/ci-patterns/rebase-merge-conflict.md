...
- `workflow` is a sync or preview workflow (see `.github/HANDLE_CI_FAILURE.md` for names).
...
(Branch name and upstream remote are in `.github/HANDLE_CI_FAILURE.md`.)
...
**Auto-generated files:** Never resolve by hand. Check `.github/HANDLE_CI_FAILURE.md` for the project's regeneration command. The general pattern is:
...
<regeneration command from the context file>
...
```bash
<check command from the context file>
```
