...
never ambient to jobs that do not ask for it, and that required reviewers or a branch policy can be
attached later without moving the secret.

The job sets `deployment: false`, so naming the environment does not file a deployment record. The
default files one per sync cycle, and the Actions run history already records the same thing.
Secrets, wait timers and required reviewers are unaffected. Custom deployment protection rule apps
are the one incompatibility — they require a deployment object, and a job that declines to create
